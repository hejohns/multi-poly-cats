{-# OPTIONS --safe #-}
module Cubical.Categories.Constructions.Free.Cartesian.Data where
...
module _ {Q : ProductQuiver ℓq ℓq'}{𝓒 : BinaryCartesianCategory ℓc ℓc'}{𝓓 : BinaryCartesianCategory ℓd ℓd'}(F G : StrictCartesianFunctor 𝓒 𝓓)(ı : Interp Q 𝓒) where
  agree-on-product-expr = {!!}
  record ∘I≡ : Type _ where
    field
      agree-on-vertex : (F ∘I ı) .I-ob ≡ (G ∘I ı) .I-ob
      agree-on-edge : PathP {!!} ((F ∘I ı) .I-hom) ((G ∘I ı) .I-hom)
