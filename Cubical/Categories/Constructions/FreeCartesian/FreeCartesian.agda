...
module Construction (G : Graph ℓ̬ ℓₑ) where
    ...
    UnivalentFreeCartesianCategory = record { cat = YonedaImage (CartesianCategory.cat FreeCartesianCategory) ; finite-products = λ J' obs → record { vertex = {!ToYonedaImage (CartesianCategory.cat FreeCartesianCategory) ⟅ (Π J' obs) ⟆!} ; element = {!!} ; universal = {!!} } }
